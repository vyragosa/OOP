...
void Base_Class::Add_To_Slave(Base_Class * _Slave_Ptr) {
	if (_Slave_Ptr != nullptr) {
		Slave_Vec.push_back(_Slave_Ptr);
	}
}
...
void Base_Class::Remove_From_Slave(std::string _Slave_Name)
{
	for (int i = 0; Slave_Vec.size(); i++) {
		if (Slave_Vec[i]->Get_Object_Name() == _Slave_Name) {
